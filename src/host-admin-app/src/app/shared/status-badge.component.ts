 ...
 * Pure utility-class styling (Tailwind) per the host-portal conventions — no component-scoped CSS.
 */
@Component({
...
export class StatusBadgeComponent {
  /** Raw status value (e.g. tenant/subscription/health status). */
  readonly status = input.required<string>();

  protected readonly tone = computed<'good' | 'warn' | 'bad' | 'neutral'>(() => {
    const s = this.status().toLowerCase();
    if (['active', 'healthy', 'trialing', 'succeeded'].includes(s)) return 'good';
    if (
      ['provisioning', 'degraded', 'past_due', 'pending_cancellation', 'processing'].includes(s)
    )
      return 'warn';
    if (['suspended', 'unhealthy', 'cancelled', 'decommissioned', 'failed'].includes(s))
      return 'bad';
    return 'neutral';
  });
